...
//     rejoinReason,
//     regNo,
//     employeeCategory, department, aadharNo, employeeId,
//     finYearId
//   };
...
//     if (data.name && data.gender && data.dob && data.employeeCategory && data.department && data.joiningDate && data.mobile && data.localAddress && data.permCity && data.permPincode && data.aadharNo && data.employeeId) {
//       return true;
//     }
//     return false;
//   }
...
//           isOpen={form}
//           form={form}
//           widthClass={"w-[95%] max-w-6xl h-[95vh]"}
//           onClose={() => {
//             setForm(false);
//             setErrors({});
//           }}
...
export default function Form() {
  ...
  const getRegNo = useCallback(() => {
    if (id || isLoading || isFetching) return;
    if (allData?.Regno) setRegNo(allData?.Regno);
  }, [allData, isLoading, isFetching, id]);

  useEffect(getRegNo, [getRegNo, id]);
  ...
  const syncFormWithDb = useCallback((data) => {
    if (!id) {
      setPanNo(""); setName(""); setFatherName(""); setDob("");
      setChamberNo(""); setlocalAddress(""); setLocalCity("");
      setLocalPincode(""); setMobile(""); setDegree("");
      setSpecialization(""); setSalaryPerMonth(""); setCommissionCharges("");
      setGender(""); setRegNo(""); setJoiningDate(""); setPermAddress("");
      setPermCity(""); setPermPincode(""); setEmail(""); setMaritalStatus("");
      setConsultFee(""); setAccountNo(""); setIfscNo(""); setbranchName("");
      setBloodGroup(""); setDepartment(""); setImage(null);
      setEmployeeCategory(""); setPermanent(""); setActive(true);
      setLeavingDate(""); setLeavingReason(""); setCanRejoin(false);
      setRejoinReason(""); setAadharNo(""); setBankName("");
      setDesignation(""); setEmployeeId("");
      return;
    }
    setPanNo(data?.panNo || "");
    setName(data?.name || "");
  ...
  const data = {
    branchId: secureLocalStorage.getItem(sessionStorage.getItem("sessionId") + "currentBranchId"),
    panNo, name, fatherName, dob, chamberNo, localAddress, localCity, localPincode,
    mobile, degree, specialization, salaryPerMonth, commissionCharges, gender,
    joiningDate, permAddress, permCity, permPincode, email, maritalStatus,
    consultFee, accountNo, ifscNo, branchName, bloodGroup,
    ...(department && { department }),
    employeeCategoryId: employeeCategory,
    permanent, active, id, leavingReason, leavingDate, canRejoin, rejoinReason,
    regNo, employeeCategory, department, aadharNo, finYearId, bankName,
    designation, employeeId,
  };

  // ✅ Validate and set errors for highlighting
  const validateData = (data) => {
    let newErrors = {};
  ...
  const handleSubmitCustom = async (callback, data, text, nextProcess) => {
    try {
      ...
      else if (!image) formData.append("isDeleteImage", true);
  ...
  const deleteData = async (id) => {
    if (id) {
      if (!window.confirm("Are you sure to delete...?")) { setForm(false); return; }
      try {
        await removeData(id).unwrap();
        setId("");
        dispatchInvalidate();
  ...
  const submitLeavingForm = () => {
    if (id) handleSubmitCustom(updateData, data, "Updated");
    else handleSubmitCustom(addData, data, "Added");
    setLeavingForm(false);
  };
