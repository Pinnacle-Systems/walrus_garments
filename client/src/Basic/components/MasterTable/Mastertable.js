...
const ACTIVE = (
    ...
    <div className='bg-white p-0.5  d-inline-block rounded-circle border-2 border-green-500 shadow-inner shadow-gray-300 my-1 text-gray-500'>
        <Power size={10} />
    </div>
...
const INACTIVE = (
    ...
    <div className='bg-white p-0.5 d-inline-block rounded-circle border-2 border-red-500 text-gray-500 my-1'>
        <Power size={10} />
    </div>
                                        ...
                                        {currentData?.map((dataObj, index) => (
                                            ...
                                            <tr key={index} className="border-b border-blue-gray-200 cursor-pointer  " >
                                                {tableDataNames?.map((data, idx) => (
                                                    ...
                                                    <td key={idx} className={`h-[30px]  border-r border-blue-gray-200 text-[11px]  ${data === "dataObj.regNo" ? "font-semibold" : "font-normal"} ${(data === "dataObj.active ? ACTIVE : INACTIVE") && (header === "Employees list") ? "ps-[65px]" : `${data === "dataObj.active ? ACTIVE : INACTIVE" ? "ps-[35px]" : "px-4"}`} `}
                                                        onClick={() => { onDataClick(dataObj.id) }}
                                                        style={{ backgroundColor: data === "dataObj.color" ? eval("dataObj.pantone") : undefined }}>
                                                        {eval(data)}
