...
import { useAddAccessoryTemplateMasterMutation, useDeleteAccessoryTemplateMasterMutation, useGetAccessoryTemplateMasterByIdQuery, useGetAccessoryTemplateMasterQuery, useUpdateAccessoryTemplateMasterMutation } from "../../../redux/uniformService/AccessoryTemplateMasterServices";
...
export default function Form() {
    ...
    const { data: allData, isLoading, isFetching } = useGetAccessoryTemplateMasterQuery({ params, searchParams: searchValue });
    ...
    } = useGetAccessoryTemplateMasterByIdQuery(id, { skip: !id });

    const [addData] = useAddAccessoryTemplateMasterMutation();
    const [updateData] = useUpdateAccessoryTemplateMasterMutation();
    const [removeData] = useDeleteAccessoryTemplateMasterMutation();

    const syncFormWithDb = useCallback((data) => {
        setName(data?.templateName ? data?.templateName : "");
        setTemplateItems(data?.AccessoryTemplateItems ? data?.AccessoryTemplateItems : []);
    ...
    const data = {
        ...
        templateItems: templateItems.filter(temp => temp.accessoryId)
    }
    ...
    const columns = [
...

        {
            ...
            accessor: (item) => item?.templateName,
    ...
    return (
        ...
        <div onKeyDown={handleKeyDown} className="p-1 h-[90%]">
            <div className="w-full flex bg-white p-1 justify-between  items-center">
                <h5 className="text-2xl font-bold text-gray-800">Accessory Template Master</h5>
                                        ...
                                        <div className="bg-white p-3 rounded-md border border-gray-200 h-full">
                                            <div className="space-y-4 ">
                                                ...
                                                <fieldset className=' rounded-tr-lg rounded-bl-lg rounded-br-lg my-5 w-full flex h-[330px] overflow-auto border border-gray-200'>
                                                    <legend className='sub-heading'>AccessoryTemplate Template Details</legend>
                                                    <TaxTemplateGrid params={params} templateItems={templateItems} setTemplateItems={setTemplateItems} readOnly={readOnly}  accessoryList={accessoryList} 
                                                    accessoryCategoryList={accessoryCategoryList} accessoryGroupList={accessoryGroupList}
                                                    />
                                                </fieldset>
                                            </div>
                                        </div>
