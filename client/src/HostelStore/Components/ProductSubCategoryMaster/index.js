...
export default function Form() {
  ...
  const { data: allData, isLoading, isFetching } = useGetProductSubCategoryQuery({ params, searchParams: searchValue });
  const { data: singleData, isFetching: isSingleFetching, isLoading: isSingleLoading } = useGetProductSubCategoryByIdQuery(id, { skip: !id });

  const [addData] = useAddProductSubCategoryMutation();
  const [updateData] = useUpdateProductSubCategoryMutation();
  const [removeData] = useDeleteProductSubCategoryMutation();
  ...
  const data = {
    name, productCategoryId,companyId: secureLocalStorage.getItem(sessionStorage.getItem("sessionId") + "userCompanyId"), active, id
  }
            ...
            <div className='mr-1 md:ml-2'>
              <fieldset className='frame my-1'>
                ...
                <div className='grid grid-cols-1 my-2'>
                  ...
                  <TextInput name="Sub Category Name" type="text" value={name} setValue={setName} required={true} readOnly={readOnly} disabled={(childRecord.current > 0)} />
