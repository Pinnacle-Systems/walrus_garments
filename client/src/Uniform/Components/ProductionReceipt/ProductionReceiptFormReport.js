...
const ProductionReceiptFormReport = ({
  ...
  const { data: allData, isLoading, isFetching } = useGetProductionReceiptQuery({ params: { branchId, ...searchFields, pagination: true, dataPerPage, pageNumber: currentPageNumber, finYearId } });
            ...
            <tbody className="border-2">
              {allData.data.map((dataObj, index) => (
                <tr
                  ...
                  <td className='py-1'>{dataObj?.Supplier?.name ? dataObj?.Supplier?.name : "InHouse"}</td>
...
export default ProductionReceiptFormReport