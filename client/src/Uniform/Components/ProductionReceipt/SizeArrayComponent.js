...
const SizeArrayComponent = ({ findReadOnly, markRead, value, valueIndex, index, setProductionReceiptDetails,
    classItem, classId,
    ...
    const handleInputChange = (value, index, sizeIndex, classItemData, sizeId, field, readyToInwardFeMaleSet, readyToInwardMaleSet, orderQtyForMale, orderQtyForFemale) => {
    ...
    const addItem = (sizeId, classItemData, val) => {
        ...
        setProductionReceiptDetails(prev => {
            ...
            newInwardDetails.push({
                ...
                maleInwardQty: val?.orderQtyForMale ? val?.orderQtyForMale : 0,
                orderQtyForMale: val?.orderQtyForMale,
                orderQtyForFemale: val?.orderQtyForFemale,
                femaleInwardQty: val?.orderQtyForFemale ? val?.orderQtyForFemale : 0,
                            ...
                            <td className="border border-gray-500 text-xs">
                                ...
                                <div className="grid grid-cols-8">
                                    ...
                                    <div className='  border-r-2 text-right '>
                                        ...
                                        {parseInt(value?.readyToInwardCommonSet) || 0}
                                    </div>
                        ...
                        <>
                            <td className=" text-xs text-center border border-gray-500">
                                <div className="grid grid-cols-6">
                                    ...
                                    <div className=' break-words border-r-2 overflow-wrap'>
                                    ...
                                    <div className='break-words  border-r-2'>
                                        {
                                            value?.male?.map((val, valIndex) => (
                                                findFromList(val?.isMaleColorId, colorList?.data, "name")
                                            )).join(",")
                            ...
                            <td className=" text-xs text-center border border-gray-500">
                                <div className="grid grid-cols-6 w-full">
                                    ...
                                    <div className=' border-r-2 text-right '>
                                        {value?.orderQtyForFemale || 0}
                                    </div>
                                    <div className=' border-r-2 text-right '>
                                        {value?.readyToInwardFeMaleSet || 0}
                            ...
                            {/* <td className="border border-gray-500 text-xs  h-7">{value?.size}</td>
                            <td className="border border-gray-500 text-xs  h-7">{value?.orderQtyForMale}</td>
                            <td className="border border-gray-500 text-xs  h-7">{value?.readyToInwardMaleSet || 0}</td>
                            <td className="border border-gray-500 text-xs  h-7">
                                <input
                                    type="number"
                                    onFocus={(e) => e.target.select()}
...
export default SizeArrayComponent
