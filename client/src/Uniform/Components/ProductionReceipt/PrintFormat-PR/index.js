...
export default function PrintFormat({ groupData,productionReceiptDetails,id, singleData, findFromList, 
  ...
  function getTotals(field) {
    const total =productionReceiptDetails?.reduce((accumulator, current) => {
  ...
  function getTotalAmount(qty, price, tax, discountType, disAmount) {
    const total = singleData?.productionConsumptionDetails?.reduce((accumulator, current) => {
...
function getReceivedQty() {
  ...
  return (
    <Document>
    <ProductionReceipt  singleData={singleData}  docId={docId} styles={styles} findAmount={findAmount} getTotals={getTotals}
     ...
     classList={classList} findFromList={findFromList} panelList={panelList} itemList={itemList} sizeList={sizeList} id={id} colorList={colorList}
      getItemId={getItemId}  getReceivedQty={getReceivedQty}
