...
const ExpenseEntryForm = ({ onClose, id, setId, docId, setDocId, date, setDate, readOnly, setReadOnly, transType, setTransType,
    ...
    const [addData] = useAddExpenseEntryMutation();
    const [updateData] = useUpdateExpenseEntryMutation();
    ...
    const saveData = (nextProcess) => {
        ...
        let mandatoryFields = ["expenseCategoryId", "amount"];

        if (!isGridDatasValid((data?.expenseEntryItems)?.filter(i => i.expenseCategoryId), false, mandatoryFields)) {
            Swal.fire({
