...
import { useGetStyleMasterByIdQuery, useGetStyleMasterQuery } from '../../../redux/uniformService/StyleMasterService';
import { showEntries } from '../../../Utils/DropdownData';
import ReactPaginate from 'react-paginate';


const FabricStockFillGrid = ({ setLocalRawMaterials, getIssuedProperty, styleId, localRawMaterials, storeId, getTotalQty }) => {
        ...
        setLocalRawMaterials(localInwardItems => {
            return localInwardItems.filter(item =>
                !(removeItem.fabricId === item.fabricId
                    ...
                    &&
                    (removeItem?.lotNo ? removeItem?.lotNo : null) === (item?.lotNo ? item?.lotNo : null)
                    // &&
    ...
    function isItemChecked(checkItem) {
        let item = localRawMaterials.find(item =>
            ...
            &&
            (checkItem?.lotNo ? checkItem?.lotNo : null) === (item?.lotNo ? item?.lotNo : null)
            // &&
    ...
    const styleOnColor = styleData?.StyleOnColor ? styleData?.StyleOnColor : [];
    ...
    const { data: stockData, isLoading, isFetching
        ...
        params: {
            branchId, itemType: "DyedFabric",
            isGetStockReport: true,
            ...searchFields,
    ...
    return (
        <div className='tx-table-cell bg-gray-200'>
            ...
            <table className='w-full text-xs'>{console.log(stockData, "stockData")}
                <thead>
                ...
                <tbody>
                    <tr className='sticky top-5 tx-table-row'>
                        ...
                        <td className='tx-table-cell  shadow-xl'>
                            <input placeholder="Search" className='w-full p-2 ' type="text" onFocus={(e) => e.target.select()}
                                onChange={(e) => {
                                    setSearchFabricAliasName(e.target.value);
                                }} />
                        </td>
                        <td className=' tx-table-cell  shadow-xl'>
                            <input placeholder="Search" className='w-full p-2' type="text"
                        ...
                        <tr key={index} className='py-2 tx-table-row'
                            onClick={() => {
                                handleCheckBoxChange(!isItemChecked(item), {
                                    ...item,
                                    stockQty: item.stockQty, stockRolls: item.stockRolls,
                    ...
                    <tr className='bg-blue-200 w-full font-bold'>
                        ...
                        <td className="tx-table-cell   w-10"></td>
                        <td className="tx-table-cell   w-10"></td>
                        <td className="tx-table-cell   w-10 text-right">{getTotalQty()}</td>
                    </tr>
                </tbody>
            </table>
            <ReactPaginate
