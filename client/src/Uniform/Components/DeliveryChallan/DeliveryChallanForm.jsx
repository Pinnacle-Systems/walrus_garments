...
import SalesInvoiceItems from "./DeliveryChallanItems";
...
const DeliveryChallanForm = ({ onClose, id, setId, docId, setDocId, date, setDate, readOnly, setReadOnly, transType, setTransType,
  ...
  const saveData = (nextProcess) => {
    ...
    let mandatoryFields = ["itemId", "uomId", "qty",];
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 overflow-visible">
            ...
            <TransactionHeaderSection title="Basic Details" className="col-span-1" bodyClassName={`${saleOrderDocId ? "grid-cols-12" : "grid-cols-2"}`}>
              <div className={saleOrderDocId ? "col-span-4" : ""}>
                <ReusableInput label="Delivery Challan No" readOnly value={docId} />
              </div>
              <div className={saleOrderDocId ? "col-span-4" : ""}>
                <ReusableInput label="Delivery Challan Date" value={date} type="date" required readOnly disabled />
              </div>
