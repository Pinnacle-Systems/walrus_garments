...
import ProductionDeleviry from './ProductionDelivery';


export default function PrintFormat({ data, singleData, date, docId, cuttingDeliveryDetails }) {
  ...
  function getTotals(field) {
    const total = singleData?.productionDeliveryDetails?.reduce((accumulator, current) => {
  ...
  return (
    <Document>
    <ProductionDeleviry  singleData={singleData}  docId={docId} styles={styles} findAmount={findAmount} getTotals={getTotals} />
    </Document>
  );
}
