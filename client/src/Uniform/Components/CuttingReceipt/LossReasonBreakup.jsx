...
import { params, substract } from '../../../Utils/helper';
import { Delete } from '../../../Buttons';
import { PLUS } from '../../../icons';
import { toast } from 'react-toastify';

const LossReasonBreakup = ({ currentIndex, setCurrentSelectedIndex, cuttingReceiptFabricConsumption, setCuttingReceiptFabricConsumption,
    readOnly, id, cuttingReceiptFabricConsumptionFillData }) => {
    const handleInputChange = (value, index, field) => {
        const newBlend = structuredClone(cuttingReceiptFabricConsumption);
        newBlend[currentIndex]["lossDetails"][index][field] = value;
        setCuttingReceiptFabricConsumption(newBlend);
    };
    function removeLotNo(index) {
        setCuttingReceiptFabricConsumption(prev => {
    ...
    useEffect(() => {
        setCuttingReceiptFabricConsumption(prev => {
            ...
            newItem[currentIndex]["lossDetails"] = [{ lossReasonId: "", lossQty: "" }]
            return newItem
        })
    }, [])

    const currentConsumption = cuttingReceiptFabricConsumption[currentIndex]
    let cuttingDeliveryItem = cuttingReceiptFabricConsumptionFillData.find(i => parseInt(i.id) === parseInt(currentConsumption.cuttingDeliveryDetailsId))
                            ...
                            <td className='text-center table-data'>
                                <input
                                    ...
                                    onChange={(event) => {
                                        ...
                                        if ((parseFloat(event.target.value) + parseFloat(currentConsumption.consumption) + totalLossQty) > balanceQty) {
