        ...
        <header className="hidden md:flex h-10 bg-white border-b border-slate-200 px-4 items-center shrink-0 z-30 justify-between shadow-sm">
            <div className="flex items-center gap-4 flex-1">
                ...
                <div ref={searchContainerRef} className="flex-1 max-w-xl relative mt-1">
                    ...
                    <input
                        ref={scannerRef}
                        disabled={(isBarcodeLoading && searchMode === 'BARCODE') || (currentBilStatus == "PAID")}
                        ...
                        className={`w-full pl-10 pr-4 py-1 rounded-lg text-sm transition-all font-medium border outline-none ${((isBarcodeLoading && searchMode === 'BARCODE') || selectedReportSaleId) ? ' border-slate-200 text-slate-400 ' : 'bg-slate-50 border-slate-200 focus:bg-white focus:border-indigo-500 placeholder:text-slate-400'}`}
                        value={searchQuery}
                        onChange={(e) => {
