...
import PurchaseGeneral from './PurchaseGeneral';


export default function PrintFormat({ groupData,productionReceiptDetails,id, singleData, findFromList, 
  docId,classList,panelList,itemList,sizeList ,colorList,isStitching,substract}) {
  ...
  function getTotals(field) {
    const total =singleData?.GeneralPurchaseItems?.reduce((accumulator, current) => {
...
function getReceivedQty() {
  ...
  return (
    <Document>
    <PurchaseGeneral  singleData={singleData}  docId={docId} styles={styles} findAmount={findAmount} getTotals={getTotals}
     productionReceiptDetails={productionReceiptDetails}  groupData={groupData} 
     classList={classList} findFromList={findFromList} panelList={panelList} itemList={itemList} sizeList={sizeList} id={id} colorList={colorList}
      getItemId={getItemId}  getReceivedQty={getReceivedQty}  getGross={getGross}
      isStitching={isStitching} substract={substract}   getTotalAmount={getTotalAmount}
     />
    </Document>
  );
}
