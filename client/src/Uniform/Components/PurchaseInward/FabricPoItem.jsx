...
const FabricPoItem = ({ fabricList, uomList,
    ...
    return (
        <>
            ...
            <tr key={item.poItemId} className='tx-table-row'>{console.log(gsmList, "gsmList")}
                ...
                <td className='text-right px-1  tx-table-cell'>{item?.balanceQty || 0}</td>
                ...
                <td className='text-right   tx-table-cell'>{(parseFloat(item?.price) * parseFloat(sumArray(item?.inwardLotDetails ? item?.inwardLotDetails : [], "qty"))).toFixed(2)}</td>
                {!readOnly &&
