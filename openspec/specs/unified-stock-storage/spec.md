# unified-stock-storage Specification

## Purpose
Define `stock` as the steady-state operational inventory store and phase out `legacyStock` as a runtime routing boundary.
...
### Requirement: Operational stock rows SHALL remain granularity-aware rather than legacy-aware
Operational stock storage SHALL continue to use the resolved item identity plus the dimensions required by stock-maintenance controls. The stock table SHALL NOT require a dedicated legacy flag to distinguish legacy item stock from canonical item stock.

#### Scenario: Legacy and canonical stock share the same stock contract
- **WHEN** the system saves stock for either a legacy item or a canonical item
- **THEN** it persists the row using the same stock-table contract for item identity and required dimensions
- **AND** it does not add a separate stock-level legacy mode to describe the row
