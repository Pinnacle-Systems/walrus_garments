...
- [x] 3.1 Verify bulk import still saves successfully when no colors are missing.
- [x] 3.2 Verify imports with missing sizes but no missing colors still complete through the lightweight batch review path.
- [x] 3.3 Verify imports with missing colors require code entry, reject duplicates, create the reviewed colors, and reuse the created ids in the saved stock rows.
