...
- [ ] 1.2 Update shared stock-writing validation helpers so configured stock-driven fields are treated as required runtime stock keys when active in Stock Control.
...
- [x] 2.2 Extend Opening Stock validation so configured runtime stock-driven fields across `field1` through `field10` block save when blank.
...
- [ ] 5.2 Verify Opening Stock, Stock Adjustment, and Stock Transfer all block save when any configured stock-driven field is missing.
- [ ] 5.3 Verify persisted `Stock` rows from Opening Stock, Stock Adjustment, and Stock Transfer include values for every configured runtime stock-driven field.
