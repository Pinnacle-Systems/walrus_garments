...
- Preserve the current database model and focus on field derivation, validation, and payload consistency.
...
### Treat configured stock-driven fields as runtime-required stock keys
When Stock Control configures a stock-driven field for runtime stock entry, stock-writing screens SHALL treat that field as part of the required stock row identity/completeness contract before save. Shared dimensions and stock-defined attributes remain different in sourcing, but both are required when active.

Why:
- The governing specs already make Stock Control the authority for runtime stock-entry completeness.
- Partial support creates silent mismatches where some flows persist complete stock rows and others do not.

Alternative considered:
- Continue treating stock-defined fields as advisory on some screens. Rejected because it breaks the requirement that all stock-writing surfaces persist complete stock keys.
...
### Reuse screen-specific value sourcing while standardizing completeness
The shared runtime contract SHALL standardize which fields are present and required, but each screen may continue to use the most appropriate sourcing pattern:
- shared dimensions may use item-scoped options or master-backed fallback
- stock-defined attributes may use direct row entry
- imported or barcode-assisted rows may start partially filled and complete before save when the workflow supports that

Why:
- It preserves existing usability patterns where they already work.
- It avoids forcing all screens into one identical editor implementation while still enforcing the same save contract.
...
- [Broader requiredness can block saves for workflows that previously allowed incomplete rows] -> Mitigation: make the requirement explicit in the specs and implement clear row-level validation messaging on each screen.
...
3. Update Stock Adjustment to render, validate, and persist the configured stock-driven field set in both adjustment rows and resulting `Stock` rows.
4. Update Stock Transfer to render, validate, and persist the configured stock-driven field set on transfer rows and both transfer stock legs.
