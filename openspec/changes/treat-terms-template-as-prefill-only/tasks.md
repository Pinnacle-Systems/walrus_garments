...
- [ ] 3.1 Smoke test quotation, sale order, sales invoice, sales delivery, and sales return to confirm templates prefill terms correctly and saved transactions reopen with their saved text snapshot.
- [ ] 3.2 Verify that changing templates after manual edits does not silently discard existing terms text and that the replacement confirmation behaves correctly.
- [ ] 3.3 Verify that printed and read-only transaction views continue to show the saved final transaction terms text rather than live template-master content.
