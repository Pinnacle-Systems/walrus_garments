...
async function getNextDocId(branchId, shortCode, startTime, endTime) {
    let lastObject = await prisma.generalPurchase.findFirst({
        where: {
    ...
    let newDocId = `${branchObj.branchCode}${getYearShortCode(new Date())}/GNP/1`
    if (lastObject) {
        newDocId = `${branchObj.branchCode}${getYearShortCode(new Date())}/GNP/${parseInt(lastObject.docId.split("/").at(-1)) + 1}`
    }
...
async function get(req) {
    ...
    let data = await prisma.generalPurchase.findMany({
        where: {
            AND: [
                {
                    AND: (finYearDate) ? [
...
async function getOne(id) {
    const childRecord = 0;
    let generalPurchase = await prisma.generalPurchase.findUnique({
        where: {
            id: parseInt(id)
        },
        include: {
    ...
    if (!generalPurchase) return NoRecordFound("generalPurchase");
    return { statusCode: 0, data: { ...generalPurchase, ...{ childRecord } } };
}

async function getSearch(req) {
    const { companyId, active } = req.query
    const { searchKey } = req.params
    const data = await prisma.generalPurchase.findMany({
        where: {
            country: {
...
export async function getPoItems(req) {
    ...
    let totalCount;
    if (pagination) {
        data = await prisma.generalPurchaseItems.findMany({
            where: {
                Po:
                {
                    branchId: branchId ? parseInt(branchId) : undefined,
...
export async function getPoItemById(id, purchaseInwardReturnId, stockId, storeId, billEntryId, poType) {
    ...
    let data = await prisma.generalPurchaseItems.findUnique({
        where: {
            id: parseInt(id)
        },
        include: {
...
async function create(body) {
    ...
    const data = await prisma.generalPurchase.create({
        data: {
            transType ,
...
async function update(id, body) {
    ...
    const dataFound = await prisma.generalPurchase.findUnique({
        where: {
            id: parseInt(id)
        },
        include: {
            GeneralPurchaseItems: true
        }
    })
    if (!dataFound) return NoRecordFound("generalPurchase");
    ...
    const isAlreadyItemAdded = id => {
        let item = dataFound.GeneralPurchaseItems.find(item => parseInt(item.id) === parseInt(id))
    ...
    let deletedItems = dataFound.GeneralPurchaseItems?.filter(item => {
        return !(poItems?.filter(item => item?.id).some(poItem => parseInt(poItem.id) === parseInt(item.id)))
    }).map(item => parseInt(item.id))
    let data;

    await prisma.$transaction(async (tx) => {
        data = await tx.generalPurchase.update({
            where: {
                id: parseInt(id),
            },
            data: {
        ...
        const updatePoItemsFunc = async () => {
            let promises = updatePoItems.map(async (item) => {
                return await tx.GeneralPurchaseItems.update({
                    where: {
                        id: parseInt(item.id)
                    },
        ...
        await tx.GeneralPurchaseItems.deleteMany({
            where: {
                id: {
                    in: deletedItems
                }
...
async function remove(id) {
    const data = await prisma.generalPurchase.delete({
        where: {
            id: parseInt(id)
        },
    })
