...
async function getNextDocId(branchId, shortCode, startTime, endTime) {
    ...
    let newDocId = `${branchObj.branchCode}${getYearShortCode(new Date())}/RFM/1`
    if (lastObject) {
        newDocId = `${branchObj.branchCode}${getYearShortCode(new Date())}/RFM/${parseInt(lastObject.docId.split("/").at(-1)) + 1}`
    }
    return newDocId
}
