...
async function get(req) {
    ...
    let data = await prisma.socksType.findMany({
        where: {
...
async function getOne(id) {
    const childRecord = 0;
    const data = await prisma.socksType.findUnique({
        where: {
            id: parseInt(id)
        }
    })
...
async function create(body) {
    ...
    const data = await prisma.socksType.create(
        {
            data: {
...
async function update(id, body) {
    ...
    const dataFound = await prisma.socksType.findUnique({
        where: {
            id: parseInt(id)
        }
    })
    if (!dataFound) return NoRecordFound("size");
    const data = await prisma.socksType.update({
        where: {
            id: parseInt(id),
        },
        data:
...
async function remove(id) {
    const data = await prisma.socksType.delete({
        where: {
            id: parseInt(id)
        },
    })
