...
import { get as _get, getOne as _getOne, getSearch as _getSearch, create as _create, update as _update, remove as _remove, styleImageUpload as _styleImageUpload, removeStyleImage as _removeStyleImage } from '../services/sample.service.js';
...
async function styleImageUpload(req, res, next) {
    try {
        res.json(await _styleImageUpload(req));
        console.log(res.statusCode);
    } catch (error) {
        console.error(`Error`, error.message);
        if (error instanceof Prisma.PrismaClientKnownRequestError) {
...
async function removeStyleImage(req, res, next) {
    try {
        res.json(await _removeStyleImage(req.params.id));
        console.log(res.statusCode);
    } catch (error) {
        if (error.code === 'P2025') {
            res.statusCode = 200;
