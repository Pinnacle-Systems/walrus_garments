...
async function getAlreadyInwardDataWithoutLimit(poInwardOrDirectInward, poItemsId, directItem) {
    ...
    if (poInwardOrDirectInward == "DirectReturn") {
        sql = `
    select  sum(AccessoryInwardItems.qty) as alreadyInwardedQty from AccessoryInward left join AccessoryInwardItems
  on AccessoryInward.id=AccessoryInwardItems.AccessoryInwardId  
  WHERE  AccessoryInward.poInwardOrDirectInward="DirectInward"  AND AccessoryInwardItems.id=${directItem?.accessoryInwardItemsId}
     `
    }
        ...
        select  sum(AccessoryInwardItems.qty) as alreadyInwardedQty from AccessoryInward left join AccessoryInwardItems
      on AccessoryInward.id=AccessoryInwardItems.AccessoryInwardId
      WHERE  AccessoryInwardItems.poItemsId=${poItemsId}
         `
    }
    else {
        sql = `
    select  sum(AccessoryInwardItems.qty) as alreadyInwardedQty from AccessoryInward left join AccessoryInwardItems
  on AccessoryInward.id=AccessoryInwardItems.AccessoryInwardId
  WHERE  AccessoryInward.poinwardordirectinward="PurchaseInward"  AND AccessoryInwardItems.poItemsId=${poItemsId}
     `
    }
...
async function getAlreadyReturnData(poInwardOrDirectInward, directReturnItemsId, poItemsId, directItem) {
    ...
    console.log(directReturnItemsId, "directReturnItemsId");
    ...
    if (poInwardOrDirectInward == "DirectReturn") {
        sql = `
        select  sum(qty) as alreadyReturnedQty,sum(noofrolls) as alreadyReturnedRolls from AccessoryReturn left join AccessoryReturnItems
      on AccessoryReturn.id=AccessoryReturnItems.accessoryReturnId
      WHERE  AccessoryReturn.poInwardOrDirectInward="${poInwardOrDirectInward}" AND  AccessoryReturnItems.accessoryPoItemsId = ${directItem?.accessoryPoItemsId} AND AccessoryReturnItems.ID < ${directItem?.id}
         `
    }
    else {
        sql = `
        select  sum(qty) as alreadyReturnedQty,sum(noofrolls) as alreadyReturnedRolls from AccessoryReturn left join AccessoryReturnItems
      on AccessoryReturn.id=AccessoryReturnItems.accessoryReturnId
      WHERE  AccessoryReturn.poInwardOrDirectInward="${poInwardOrDirectInward}" AND  AccessoryReturnItems.ID < ${directReturnItemsId} AND AccessoryReturnItems.accessoryPoItemsId=${poItemsId}
         `
    }
...
export async function getPurchaseReturnItemsAlreadyData(directReturnOrPoReturnId, poInwardOrDirectInward, poType, AccessoryReturnItems, storeId, createdAt) {
    ...
    console.log(directItemsData, "directItemsData")
